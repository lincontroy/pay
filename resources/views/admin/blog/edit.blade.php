...
<div class="row">
  <div class="col-12">
    <div class="card">
      ...
      @endif
      <form method="POST" action="{{ route('admin.blog.update', $blog_edit->id) }}" enctype="multipart/form-data" class="basicform">
        @csrf
        @method('put')
        @php
        $excerpt_info = $blog_edit->excerpt->value ?? '';
        $description_info = $blog_edit->description->value ?? '';
        $thumimg_info = $blog_edit->thum_image->value ?? '';
        @endphp
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <textarea name="short_description" cols="30" rows="10" class="form-control">{{ $excerpt_info }}</textarea>
          </div>
          <div class="form-group">
            <label>{{ __('Description') }}</label>
            <textarea name="description" class="summernote form-control">{{ $description_info }}</textarea>
          </div>
          <div class="form-group">
