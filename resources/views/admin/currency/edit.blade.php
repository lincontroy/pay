...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>'Currency Edit','button_name'=> 'All Currency','button_link'=> route('admin.currency.index')])
@endsection
...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <form method="POST" action="{{ route('admin.currency.update',$data->id) }}" class="basicform">
                @method("PUT")
                @csrf
                <div class="card-body">
                    ...
                    <div class="form-group row mb-4">
                        <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('Symbol') }} <sup>*</sup></label>
                        <div class="col-sm-12 col-md-7" >
                            <input type="text" id="price" class="form-control" name="symbol" value="{{old('symbol') ? old('symbol') :$data->symbol}}">
                        </div>
                    </div>
                    <div class="form-group row mb-4">
                        ...
                        <div class="col-sm-12 col-md-7" >
                            <select name="position" class="form-control selectric">
                                <option value="1" {{$data->position == 1 ? 'selected':""}}>{{ __('Left') }}</option>
                                <option value="0" {{$data->position == 0 ? 'selected':""}}>{{ __('Right') }}</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group row mb-4">
